...
- `options` (`Partial<RGXPartOptions<R, S, T>>`, optional): Configuration options. Defaults to `{}`.
  ...
  - `beforeCapture` (`((share: S, part: RGXPart<R, S, T>, walker: RGXWalker<R, S>) => RGXPartControl) | null`, optional): A callback invoked before capturing this part during walking. Receives the walker's current `share` value as its first argument. Returns an `RGXPartControl` value to control walker behavior: `"skip"` to skip this token without capturing, `"silent"` to capture but not record in `captures`, `"stop"` to halt immediately without capturing or advancing, or `void`/`undefined` to proceed normally. Defaults to `null`.
  - `afterCapture` (`((capture: RGXCapture<T>, share: S, part: RGXPart<R, S, T>, walker: RGXWalker<R, S>) => void) | null`, optional): A callback invoked after capturing this part during walking. Receives the typed `RGXCapture<T>` result and the walker's current `share` value. Can call `walker.stop()` to halt walking after this capture. Defaults to `null`.
  - `afterFailure` (`((e: RGXRegexNotMatchedAtPositionError, share: S, part: RGXPart<R, S, T>, walker: RGXWalker<R, S>) => void) | null`, optional): A callback invoked when the regex match for this part fails (i.e., `RGXRegexNotMatchedAtPositionError` is thrown). Receives the error, the walker's current `share` value, the part, and the walker. The error is still re-thrown after this callback runs. Defaults to `null`.
  - `afterValidationFailure` (`((e: RGXPartValidationFailedError, share: S, part: RGXPart<R, S, T>, walker: RGXWalker<R, S>) => void) | null`, optional): A callback invoked when validation fails for this part (i.e., `RGXPartValidationFailedError` is thrown). Receives the error, the walker's current `share` value, the part, and the walker. The error is still re-thrown after this callback runs. Defaults to `null`.
  - `validate` (`((capture: RGXCapture<T>, share: S, part: RGXPart<R, S, T>, walker: RGXWalker<R, S>) => boolean | string) | null`, optional): A callback invoked during validation after capturing and transforming, but before `afterCapture`. Receives the capture result, the walker's current `share` value, the part, and the walker. Returns `true` if validation passes, `false` to fail with a generic error, or a string to fail with that string as the error message. Defaults to `null`.
...
- `beforeCapture` (`((share: S, part: RGXPart<R, S, T>, walker: RGXWalker<R, S>) => RGXPartControl) | null`, readonly): The before-capture callback, or `null`.
- `afterCapture` (`((capture: RGXCapture<T>, share: S, part: RGXPart<R, S, T>, walker: RGXWalker<R, S>) => void) | null`, readonly): The after-capture callback, or `null`.
- `afterFailure` (`((e: RGXRegexNotMatchedAtPositionError, share: S, part: RGXPart<R, S, T>, walker: RGXWalker<R, S>) => void) | null`, readonly): The after-failure callback, or `null`.
- `afterValidationFailure` (`((e: RGXPartValidationFailedError, share: S, part: RGXPart<R, S, T>, walker: RGXWalker<R, S>) => void) | null`, readonly): The after-validation-failure callback, or `null`.
...
- `share` (`S`): A user-defined shared value passed to all Part callbacks. Unlike `reduced`, this is not accumulated — it is passed as-is to every callback invocation, making it suitable for shared mutable state across parts.
...
- `hasNextToken(predicate?: (token: RGXTokenOrPart<R, S>) => boolean) => boolean`: Returns `true` if there is a current token and it satisfies the optional predicate (defaults to `() => true`).
...
- `step() => RGXCapture | null`: Steps through the next token in the collection. If the token is an `RGXPart`, calls `beforeCapture` first (passing `share` as the first argument) — if it returns `"stop"`, sets `stopped` and returns `null` without advancing; if `"skip"`, advances the token position and returns `null` without capturing; if `"silent"`, captures but does not add to `captures` or `namedCaptures`. After capturing, validates (passing `share`). If validation fails, `afterValidationFailure` is called before the error is re-thrown. If the match itself fails, `afterFailure` is called before the error is re-thrown. After validating, calls `afterCapture` (passing `share`) if present. Returns the `RGXCapture` result, or `null` if there are no more tokens (or no more source in `infinite`/`looping` mode), the step was skipped, or the walker was stopped.
