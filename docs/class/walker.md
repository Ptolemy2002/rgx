...
- `options` (`Partial<RGXPartOptions<R, S, T>>`, optional): Configuration options. Defaults to `{}`.
  ...
  - `afterFailure` (`((e: RGXRegexNotMatchedAtPositionError, context: RGXPartContext<R, S, T>) => void) | null`, optional): A callback invoked when the regex match for this part fails (i.e., `RGXRegexNotMatchedAtPositionError` is thrown). Receives the error and an `RGXPartContext` object containing `part` and `walker`. The error is still re-thrown after this callback runs. Defaults to `null`.
  - `afterValidationFailure` (`((e: RGXPartValidationFailedError, context: RGXPartContext<R, S, T>) => void) | null`, optional): A callback invoked when validation fails for this part (i.e., `RGXPartValidationFailedError` is thrown). Receives the error and an `RGXPartContext` object containing `part` and `walker`. The error is still re-thrown after this callback runs. Defaults to `null`.
...
- `afterFailure` (`((e: RGXRegexNotMatchedAtPositionError, context: RGXPartContext<R, S, T>) => void) | null`, readonly): The after-failure callback, or `null`.
- `afterValidationFailure` (`((e: RGXPartValidationFailedError, context: RGXPartContext<R, S, T>) => void) | null`, readonly): The after-validation-failure callback, or `null`.
